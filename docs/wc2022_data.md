...
| Linked brush (passing) | `mark_point` + `mark_bar` | `alt_brush_passing` | Passing **volume vs accuracy** — passes attempted (x) vs completion % (y), colored by position. Two continuous measures, so it forms a full scatter cloud. Brush a box to recount the count-by-position bars, **and/or type in the player search box** to filter both views to names containing the text (any part of the full name). |
